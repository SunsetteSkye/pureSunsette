SurfingPikachuMinigame::
	...
	xor a
	ldh [rIF], a
	; PureRGBnote: all 4 interrupts ($f) incl. LCD-STAT, which drives the per-scanline wave
	; warp via the LCDC handler now installed at the $48 vector (see home/lcd.asm). rSTAT is
	; set to mode-0 (HBlank) below; the handler reads wLYOverrides[rLY] -> the HRAM reg in
	; hLCDCPointer. The minigame zeroes hLCDCPointer + rSTAT + restores rIE when it finishes.
	ld a, $f
	ldh [rIE], a
	ld a, $8
	ldh [rSTAT], a
	...
	; Sunsette: stash this run's final score somewhere stable BEFORE the cleanup below
	; (ReloadMapAfterSurfingMinigame clobbers wTileMapBackup, which wSurfingMinigameTotalScore
	; is unioned over). The Summer Beach House script reads wSurfingMinigameLastScore to pay
	; out coins when the player wagered. Copied verbatim (little-endian BCD), like TotalScore.
...
.not_high_score
	...
	; PureRGBnote: REMOVED: ldpikacry e, PikachuCry28 (Yellow PCM cry setup; helper no-op'd)
...
.high_score
	...
	call WaitForSoundToFinish
	; PureRGBnote: REMOVED: ldpikacry e, PikachuCry34 (Yellow PCM cry setup; helper no-op'd)
	call SurfingMinigame_PlayPikaCryIfSurfingPikaInParty
	ld a, SFX_GET_ITEM_2
	call PlaySound
	scf
...
; PureRGBnote: CHANGED: pokeyellow played a PCM Pikachu voice clip here (its
; PlayPikachuSoundClip uses a Yellow-only PCM sample engine pureRGB lacks). The
; trick still plays SFX_GET_ITEM2 right after this call, so we simply no-op the
; voice. Could be re-enabled with `ld a, PIKACHU / call PlayCry` if desired.
	ret
...
SurfingPikachuMinigameIntro:
	...
	call UpdateGBCPal_OBP1
	call DelayFrame
	; PureRGBnote: CHANGED: play the authentic "Pikachu's Beach" track. It isn't in the song-id
	; table, so (like the Secret Lab music) we start a real 3-channel song to claim channels 1-3,
	; then repoint each to the Pikachu's Beach data. Music_TrainerBattle is used purely as a
	; 3-channel host that shares Pikachu's Beach's bank (8); it's overwritten before it sounds.
	; The minigame's tempo system then speeds/slows these channels with the surf speed as before.
	ld a, MUSIC_TRAINER_BATTLE
	ld c, BANK(Music_TrainerBattle)
	call PlayMusic
	ld de, Music_SurfingPikachu_Ch1
	callfar Audio3_RemapChannel1
	ld de, Music_SurfingPikachu_Ch2
	callfar Audio3_RemapChannel2
	ld de, Music_SurfingPikachu_Ch3
